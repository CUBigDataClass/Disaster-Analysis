...
#2738
for x in range(1440):
    dayOneIncrementByAMinute = dayOne + incrementByAMinute
    dayOneIncrementByAMinute = dayOneIncrementByAMinute.replace(tzinfo=utc)
    ...
    output = dict(contentRdd.filter(lambda (x,y): y > dayOne and y < dayOneIncrementByAMinute).map(lambda (x,y): x).flatMap(getCount).reduceByKey(add).collect())
    result = db.minuteAnalysisLatest.insert_one({"date": dayOne , "count":output})

    #result = db.analysisData.insert_one(content)
    #print "Date: ",dayOne , " Count:" , count
    dayOne = dayOneIncrementByAMinute
